...
import { ModalFornecedoresRacComponent } from 'src/app/pages/gerenciador-atendimento-rac/manutencao/components/manutencao-aba-dois/components/modal-fornecedores/modal-fornecedores.component';
...
export class ManutencaoListFornecedorComponent implements OnInit {
  ...
  getFornecedorRaio(form: FormGroup, km: number): void {
    ...
    _this.fornecedorService.getPorRaio(form.value, 25).subscribe(res => {
      if (res.data.results.length === 0) {
        ...
        if (_this.servicosRestantes.length > 0) {
          ...
          if (!modalOpened) {
            modalOpened = true;
            const modal = _this.modalService.open(ModalFornecedoresRacComponent, { size: 'lg' });
