...
export class ManutencaoAbaUmComponent implements OnInit, AfterViewInit, AfterContentChecked {
  ...
  comDocumentacaoSinistro = false;
  comDocumentacaoSinistroAnterior = false;
  ...
  ngAfterViewInit(): void {
    ...
    _this.activatedRoute.queryParams.subscribe(params => {
      ...
      if (params['atendimentoId'] && params['tpsAt'] !== 's') {
  ...
  createForm() {
    this.form = this.formBuilder.group({
      ...
      'hideAgendamento': ['', Validators.compose([])],
      ...
      'isSemAgendamentoParada': ['', Validators.compose([])],

      'checklistServicos': ['', Validators.compose([])],
      'descricaoOcorridoGeral': ['', Validators.compose([])],
      'descricaoServicos': ['', Validators.compose([])],
      ...
      'veiculoApreendido': ['', Validators.compose([])],
  ...
  private preencherTelaEdicao() {
    ...
    this.form.get('semAgendamentoParada').setValue(atendimento.semAgendamentoParada);
  ...
  resetPage() {
    ...
    this.acessorios = [];
    this.atendimentoId = this.atendimentoId ? this.atendimentoId : null;
  ...
  validarForm() {
    ...
    const isSemAgendamentoParada = !!(execucao.find(item =>
      item.descricaoQuestionario.includes('sem agendamento de parada')));

    if (!isSemAgendamentoParada && (!preventiva.value || preventiva.value.length === 0)
    ...
    if (!this.form.valid) {
      ...
      if (!this.isRac() && this.comDocumentacaoSinistro && this.totalDocs === 0) {
        Util.validateAllFormFields(this.formArquivos);
      }
      return false;
    }

    if (!this.isRac() && this.comDocumentacaoSinistro && this.totalDocs === 0) {
      this.snackBar.open(this.translate.instant('PORTAL.MSG_ANEXO_OBRIGATORIO'), 3500, 'X');
      this.formArquivos.get('tipoArquivo').setErrors({ 'incorrect': true });
      Util.validateAllFormFields(this.formArquivos);
      return false;
    }
  ...
  fillForm() {
    ...
    this.form.get('hideAgendamento').setValue(checklistServicos.length === 0 && isSemFornecedorOuCRLV);
    ...
    const servicos = '';
    this.form.get('descricaoServicos').setValue(this.getDescricaoServicos(servicos, true));

    const execucao = this.form.get('execucao').value || [];
    this.form.get('isSemAgendamentoParada').setValue(!!(execucao
      .find(item => item.descricaoQuestionario.includes('sem agendamento'))));

    this.form.get('arquivosParaExcluir').setValue(this.arquivosParaExcluir);

    if (this.getRac()) {
      this.form.get('siglaLojaUnidas').setValue(this.getRac().siglaLoja);
    }
  ...
  getDescricaoOcorridoGeral(): string {
    ...
    let ocorrido = Util.formataData(new Date().getTime(), 'DD/MM/YYYY HH:mm:ss');
    ocorrido = ocorrido.concat(' - ' + this.userContextService.getDados().nomeUsuario + '\n');


    ocorrido = this.getDescricaoServicos(ocorrido, false);
  ...
  getDescricaoServicos(ocorrido: any, quebraLinha: boolean): string {
  ...
  changeSinistro(event?: any) {
    ...
    if (values && values.length > 0) {
      ...
      if (values.find(item => item.descricaoQuestionario.toLowerCase().includes('apreendido')
        || item.descricaoQuestionario.toLowerCase().includes('furto'))) {
        const corretivas = this.form.get('corretiva').value;
        if (corretivas && corretivas.find(item => item.descricaoQuestionario.toLowerCase().includes('via placa')
          || item.descricaoQuestionario.toLowerCase().includes('via de crlv'))) {
          this.isSegundaVia = true;
        } else {
          this.isSegundaVia = false;
        }
        this.form.get('veiculoApreendido').setValue(true);
    ...
    this.showExecucao = this.showExecucao && !this.isRac();
  }
  ...
  onFileChange(event) {
    ...
    const descricaoArquivo = this.currentDoc.name;
